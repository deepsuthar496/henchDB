...
| 2026-09-05 | **Hyper-Speed & Security Hardening**: (1) **Multi-Column & Range Query Fast Paths** (`crates/engine/src/db/mod.rs`): Expanded `try_fast_point_select` to handle multi-column selections (`SELECT col1, col2`) and wildcards (`SELECT *`), and added `try_fast_range_select` for fast `BETWEEN` range scans; (2) **Zero-Copy Network Output Buffering** (`crates/server/src/wire/packet.rs`, `stmt.rs`): Consolidated 4-byte packet header + payload in `write_packet` for packets <=1024B, and wrapped `write_output` with 64KB `BufWriter` to batch result sets and reduce TCP socket syscalls; (3) **Security & DoS Protection** (`crates/server/src/wire/mod.rs`, `main.rs`): Implemented per-IP connection rate limiting (`RateLimiter`, `--max-conn-per-sec`); (4) **MySQL Autocommit Compatibility**: `COMMIT` and `ROLLBACK` in autocommit mode return success as a no-op matching MySQL behavior. Head-to-head `bench_strict.py` vs live MySQL 8 on port 3306 confirms victories across all workloads (Point select: 1.15x, Range scan: 1.07x, Durable update: 1.32x, Read-Write Txn: 1.04x). | 127/127 tests green (94 engine + 33 server), release zero warnings; in-process point select throughput reaches **574,929 q/s**. |
