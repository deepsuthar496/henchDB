...
- **Quality & Size Ceiling**: **169/169 tests passing** (110 engine + 59 server), release builds with zero warnings, and every source file is strictly under 1,500 lines (with `sql/`, `db/`, and `wire/` cleanly modularized).
...
* **Status**: 🟡 **PLANNED**
* **Why it matters**: Complex multi-table joins, subqueries, and non-equi predicates currently rely on heuristic greedy ordering. A true enterprise engine requires statistical cost-based plan selection based on actual data distribution and selectivity.
* **Architecture**:
  - `ANALYZE TABLE <tbl>` gathering column cardinality, min/max bounds, null counts, and equidepth histograms persisted in the catalog.
  - Cost model estimating I/O and CPU costs for access paths: full scan vs. secondary index seek vs. PK point seek vs. hash join build/probe.
  - Statistical join reordering selecting optimal build vs. probe sides and index access paths based on expected selectivity rather than purely greedy table sizes.
* **Effort**: High.

---

### Verification Checklist for Any Future Changes
1. `cargo test` — all green (**169 tests: 110 engine + 59 server** as of this writing).
