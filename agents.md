...
| SQL | Hand-written lexer/parser; SELECT/INSERT/UPDATE/DELETE/DDL/BEGIN/COMMIT/ROLLBACK/SHOW TABLES/CHECKPOINT; WHERE = AND/OR/NOT + IN/BETWEEN/LIKE over column-vs-literal ANDed (parens, precedence); index access paths on PK (point/multi-point/range) + secondary; AUTO_INCREMENT integer PKs; SUM/AVG/MIN/MAX (+COUNT); INNER/LEFT JOIN + GROUP BY (nested loop, single-table hot path unchanged) | sqlparser-rs MySQL dialect, Cascades memo optimizer, morsel-driven vectorized execution (Arrow), hash joins, GROUP BY pushdown |
