...
crates/engine/src/
  ...
  db/                 Database facade split per §9 ceiling: mod (sessions,
                      ...
                      explain (EXPLAIN plan display), query (SELECT/JOIN/GROUP BY
                      execution), plan (access paths), tests/ (suites: core, joins,
                      fk, txn, opt, subquery, sysviews, batch, privilege)
