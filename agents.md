...
crates/engine/src/
  ...
  wal.rs              WAL records, CRC32, group-commit seam, recovery scan
                      (v4: Commit carries trailing commit timestamp for PITR)
  wal/shard.rs        per-core staging FIFOs + atomic offset sequencer
                      (Priority 17; syncer drains in offset order)
  archive.rs          HDBA WAL archive segments (PITR source)
  pitr.rs             point-in-time recovery (base backup + archive roll-forward)
  ...
  db/                 Database facade split per §9 ceiling: mod (sessions,
                      ...
                      indexes), dml (INSERT/UPDATE/DELETE execution + point update
                      fast paths), privilege (RBAC principals + grants + deny gate),
                      diag (SHOW STATUS/ENGINE/PROCESSLIST, Prometheus assembly),
                      cost (CBO selectivity + costing), replica (read-only gate,
                      WAL/snapshot apply, promotion), batch (morsel-driven columnar
                      execution: ColumnBatch, selection vectors, pushed-down filters
                      + aggregates), subquery (IN/EXISTS/scalar/derived evaluation),
                      sysviews (virtual pg_catalog/information_schema views +
                      version()/current_schema()/current_database()/user()),
                      explain (EXPLAIN plan display), memo (Cascades memo query optimizer:
                      equivalence classes, transformations, implementation rules, costing,
                      branch-and-bound pruning, EXPLAIN MEMO), query (SELECT/JOIN/GROUP BY
                      execution), plan (access paths), tests/ (suites: core, joins,
                      fk, txn, opt, subquery, sysviews, batch, privilege, memo, mvcc)
...
crates/server/src/
  ...
  net/                pool/poller/broker multiplexing (Priority 11):
                      conn (states), pool (bounded workers), poller (peek
                      sweep), tests (500-conn, 1040/53300, drain)
  ...
  replication/        physical WAL streaming (protocol, primary feeder,
                      read-only replica + reconnect + promotion fence) on --repl-port
...
| Writes | Session-staged write set; commit takes one commit lock, validates, WAL-batches, installs (allocating an MVCC commit epoch, advancing visible_epoch); installs record superseded rows atomically into version buffers (`record_commit_batch`) | Per-core WAL shards, Early Lock Release, column-granular versioning (RCC) |
| Durability | Single WAL file (v4: Commit carries a commit timestamp), CRC32 per record, per-txn redo on recovery, snapshot + WAL truncate checkpoint. **Group commit implemented**: commits reserve offsets atomically and stage into per-core FIFOs (`wal/shard.rs`, no file lock/syscall in the append path), one background syncer drains all shards in offset order into one write + one fsync (200us collection window), installs happen strictly in WAL-offset order (install frontier + condvar); DDL goes through the same sequencer via `Database::wal_commit`. **Continuous archiving**: with `--wal-archive-dir`, checkpoints persist the truncated durable prefix into immutable `HDBA` segments (`archive.rs`); `server restore --archive-dir [--target-time\|--target-txn]` rolls a base backup forward (`pitr.rs`) | Lock-free commit pipeline (appends still serialize under the commit lock), io_uring `IOPOLL` Linux backend at the syncer flush seam (needs Linux CI; `cfg`-gate it), parallel replay |
...
| SQL | Hand-written lexer/parser (`sql/` modules); SELECT/INSERT/UPDATE/DELETE/DDL/BEGIN/COMMIT/ROLLBACK/SHOW TABLES/CHECKPOINT; WHERE = AND/OR/NOT + IN/BETWEEN/LIKE over column-vs-literal ANDed (parens, precedence); Cascades memo optimizer (`db/memo.rs`: equivalence classes, join commutativity/associativity, predicate pushdown, Scan/HashJoin/NestedLoopJoin/Aggregate lowering, branch-and-bound pruning, `EXPLAIN MEMO`); CBO access paths (`db/cost.rs`: stats-driven PK point/range + secondary seek vs full scan on cost, ANALYZE TABLE persistence, EXPLAIN [ANALYZE]); index access paths on PK (point/multi-point/range) + secondary; AUTO_INCREMENT integer PKs; SUM/AVG/MIN/MAX (+COUNT); INNER/LEFT JOIN (hash join on equi-keys, nested-loop fallback, greedy smallest-ready-first ordering with LEFT barriers); FOREIGN KEY (RESTRICT/CASCADE/SET NULL, auto-indexed columns, PK/secondary/scan seeks); subqueries (`db/subquery.rs`: IN/EXISTS/scalar uncorrelated-cached + correlated via session frames, derived FROM/JOIN materialized ephemeral, literal projection); system catalog (`db/sysviews.rs`: read-only virtual `pg_catalog.*` / `information_schema.*` synthesized per statement as ephemeral tables, `version()`/`current_schema()`/`current_database()`/`user()` projection functions, FROM-less single-row SELECT, parse-time `::` casts, `BEGIN TRANSACTION`); morsel-driven columnar batch execution for single-table global/grouped aggregates (`db/batch.rs`: 1024-row ColumnBatch morsels, ascending selection vectors, pushed-down filters + folds, FullScan-gated with scalar fallback); RBAC (`db/privilege.rs`: principals + grant rules, deny-before-execute gate incl. fast paths, `GRANT`/`REVOKE`/`CREATE|DROP|ALTER USER`/`SHOW GRANTS`, root bypass; `auth.bin` v2 carries verifiers + grants); MVCC snapshot isolation (Priority 20: RepeatableRead default with auto-pin on first read, ReadCommitted statement-scoped snapshots, `SET [SESSION|GLOBAL] TRANSACTION ISOLATION LEVEL`, `visible_epoch` atomic visibility) | sqlparser-rs MySQL dialect, morsel-driven vectorized execution (Arrow), GROUP BY pushdown |
| Server | Bounded worker pool (`--threads`, default 2xCPU) + single poller parking idle sockets non-blocking (peek readiness, no thread per connection), dual wire frontends — MySQL (text + binary prepares, `CLIENT_SSL` optional via `--tls-cert`/`--tls-key` rustls) + PostgreSQL 3.0 on `--pg-port` (simple query + extended Parse/Bind/Describe/Execute with text/binary params, SSLRequest/TLS upgrade, cleartext auth vs `auth.bin`) — plus legacy framed text (auto-detected, `--no-legacy` to disable); `--max-connections` (default 1024; 1040/53300/`ERR` rejections) + `--wait-timeout` idle reap; COM_SHUTDOWN + signal graceful drain; `server passwd` manages `auth.bin` verifiers; `--wal-archive-dir` enables PITR archiving, `server restore --archive-dir` rolls forward; `--replica-of` streams read-only, `PROMOTE` (SQL) / `server promote --dir` (offline) fences + flips to primary | Pinned thread-per-core runtime, io_uring sockets, server-side cursors, statement timeouts, per-user privileges, PG COPY/SCRAM/cursors (PG3) |
...
- **MVCC snapshots for long readers**: Priority 20 implemented multi-version
  snapshot isolation (RepeatableRead default, ReadCommitted, consistent
  snapshots) backed by in-memory version buffers (`db/mvcc.rs`). Commits
  install atomically into the tree and advance `visible_epoch`, while readers
  time-travel across version chains with safe EBR-style pruning.
