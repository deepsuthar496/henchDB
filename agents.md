...
crates/engine/src/
  ...
  db/                 Database facade split per §9 ceiling: mod (sessions,
                      ...
                      replica (read-only gate, WAL/snapshot apply, promotion),
                      batch (morsel-driven columnar execution: ColumnBatch,
                      selection vectors, pushed-down filters + aggregates),
                      subquery (IN/EXISTS/scalar/derived evaluation),
                      sysviews (virtual pg_catalog/information_schema
                      views + version()/current_schema()/current_database()/user()),
                      explain (EXPLAIN plan display), query
                      (SELECT/JOIN/GROUP BY execution), plan (access paths),
                      tests/ (suites: core, joins, fk, txn, opt, subquery,
                      sysviews, batch)
