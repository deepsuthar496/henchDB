...
/// are valid through. Missing/corrupt file (or snapshot files) = fresh.
fn read_resume(dir: &std::path::Path) -> (u64, u64) {
    ...
    if gen == NO_GENERATION || off < LOG_START || !dir.join("snapshot.bin").exists() {
        return (NO_GENERATION, LOG_START);
    }
    (gen, off)
}

fn write_resume(dir: &std::path::Path, generation: u64, offset: u64) {
    let _ = std::fs::write(dir.join(OFFSET_FILE), format!("{generation} {offset}\n"));
}
...
pub fn run_replica(
    ...
    let (mut generation, mut applied) = read_resume(&opts.dir);
    ...
    loop {
        ...
        match stream_once(&db, &opts, &mut generation, &mut applied, &draining, global) {
            Ok(()) => {
    ...
    loop {
        ...
        match read_frame(&mut stream)? {
            ...
            Some(Frame::SnapshotEnd) => {
                ...
                apply_snapshot(db, &opts.dir, &s.buf, s.end_offset, s.generation)?;
...
fn apply_snapshot(
    ...
    write_resume(dir, generation, end_offset);
